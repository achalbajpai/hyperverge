    ...
    try {
        ...
        if (courseTasksResponse.ok) {
            ...
            if (taskWithId10) {
                ...
                const actualTaskResponse = await fetch(`${BACKEND_URL}/tasks/${taskWithId10.id}`);
                console.log(`   Fetching actual task ${taskWithId10.id}: ${actualTaskResponse.status}`);
                
                if (actualTaskResponse.ok) {
                    const actualTaskData = await actualTaskResponse.json();
                    console.log('   Actual task data:', JSON.stringify(actualTaskData, null, 2));
                }
            } else {
