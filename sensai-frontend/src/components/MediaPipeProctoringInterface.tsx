...
export default function MediaPipeProctoringInterface({
    ...
    const loadMediaPipe = async () => {
      try {
        ...
        holisticModel.setOptions({
          modelComplexity: MEDIAPIPE_OPTIMAL_CONFIG.model.modelComplexity,                    // 0 for better performance
          smoothLandmarks: MEDIAPIPE_OPTIMAL_CONFIG.model.smoothLandmarks,                   // Enable landmark smoothing (CRITICAL)
          enableSegmentation: MEDIAPIPE_OPTIMAL_CONFIG.model.enableSegmentation,             // Disabled for performance
          smoothSegmentation: MEDIAPIPE_OPTIMAL_CONFIG.model.smoothSegmentation,             // Enable if segmentation used
          refineFaceLandmarks: MEDIAPIPE_OPTIMAL_CONFIG.model.refineFaceLandmarks,           // Better face accuracy
          minDetectionConfidence: MEDIAPIPE_OPTIMAL_CONFIG.model.minDetectionConfidence,     // Increased from 0.5 to 0.7
          minTrackingConfidence: MEDIAPIPE_OPTIMAL_CONFIG.model.minTrackingConfidence        // Increased from 0.5 to 0.7
        });
  ...
  const onMediaPipeResults = useCallback((results: any) => {
    ...
    if (frameProcessingCounter.current % MEDIAPIPE_OPTIMAL_CONFIG.performance.processEveryNthFrame !== 0) return;
    ...
    try {
      ...
      canvas.width = Math.min(videoRef.current.videoWidth || 640, MEDIAPIPE_OPTIMAL_CONFIG.performance.maxProcessingWidth);
      canvas.height = Math.min(videoRef.current.videoHeight || 480, MEDIAPIPE_OPTIMAL_CONFIG.performance.maxProcessingHeight);
  ...
  const processFaceDetectionStabilized = (faceLandmarks: any[], ctx: CanvasRenderingContext2D) => {
    ...
    // Track face detection history with optimal configuration
    faceDetectionHistory.current.push(true);
    if (faceDetectionHistory.current.length > MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.stabilizationFrames) {
      faceDetectionHistory.current.shift();
    }
    ...
    // Use optimal positioning threshold from configuration
    if (distance > MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.positionThreshold) {
      createViolationOptimized('face_detection', 'medium', 'Face is not properly positioned in camera view', 
        MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.confidenceThreshold, {
        distance: Math.round(distance * 100) / 100,
        facePosition: faceCenter,
        canvasCenter,
        recommendation: 'Please center your face in the camera',
        threshold: MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.positionThreshold
      });
    }

    // Optimized face size thresholds from configuration
    const faceSize = calculateFaceSize(faceLandmarks);
    if (faceSize < MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.faceSizeMin) {
      createViolationOptimized('face_detection', 'medium', 'Face too far: move closer to camera', 0.75, {
        ...
        minThreshold: MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.faceSizeMin
      });
    } else if (faceSize > MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.faceSizeMax) {
      createViolationOptimized('face_detection', 'medium', 'Face too close: move back from camera', 0.75, {
        ...
        maxThreshold: MEDIAPIPE_OPTIMAL_CONFIG.faceDetection.faceSizeMax
      });
    }
  };

  // Optimized gaze tracking with configurable smoothing and reduced sensitivity
  const processGazeTrackingSmooth = (faceLandmarks: any[], ctx: CanvasRenderingContext2D) => {
    ...
    try {
      ...
      // Configurable frame moving average smoothing (CRITICAL for stability)
      gazeHistory.current.push(gazeVector);
      if (gazeHistory.current.length > MEDIAPIPE_OPTIMAL_CONFIG.gazeTracking.smoothingWindowFrames) {
      ...
      // Only calculate deviation if we have baseline and sufficient history
      if (!gazeBaseline.current || gazeHistory.current.length < MEDIAPIPE_OPTIMAL_CONFIG.gazeTracking.smoothingWindowFrames) return;

      // Use optimal threshold for significant gaze deviation (67% less sensitive)
      const threshold = MEDIAPIPE_OPTIMAL_CONFIG.gazeTracking.deviationThreshold;
      ...
      if (deviation > threshold) {
        ...
        createViolationOptimized('gaze_tracking', 'medium', `Gaze deviation detected: looking ${direction}`, 0.80, {
          ...
          direction,
          threshold,
          smoothingFrames: gazeHistory.current.length
        });
  ...
  // Calibrated eye tracking with user-specific EAR thresholds and optimized throttling
  const processEyeTrackingCalibrated = (faceLandmarks: any[], ctx: CanvasRenderingContext2D) => {
    ...
    try {
      ...
      // User-calibrated blink detection with optimal threshold
      ...
      // Eye movement detection with optimized threshold from configuration
      if (lastEyePosition.current) {
        ...
        // Use optimal movement threshold from configuration
        if (movement > MEDIAPIPE_OPTIMAL_CONFIG.eyeTracking.movementThreshold) {
          createViolationOptimized('eye_movement', 'low', 'Rapid eye movement pattern detected', 0.65, {
            ...
            threshold: MEDIAPIPE_OPTIMAL_CONFIG.eyeTracking.movementThreshold
          });
      ...
      // Prolonged eye closure detection with user calibration and optimal timing
      const userClosureThreshold = Math.min(userEARBaseline.current.left, userEARBaseline.current.right) * 0.5;
      const CLOSURE_TIME_THRESHOLD = MEDIAPIPE_OPTIMAL_CONFIG.eyeTracking.closureTimeThreshold;
      ...
      if (avgEAR < userClosureThreshold) {
        ...
        if (now - lastClosureTime > CLOSURE_TIME_THRESHOLD) {
          createViolationOptimized('eye_movement', 'medium', 'Prolonged eye closure detected', 0.85, {
            ...
            userBaseline: userEARBaseline.current,
            configuredThreshold: CLOSURE_TIME_THRESHOLD
          });
  ...
  // Enhanced violation creation with progressive throttling system
  const createViolationOptimized = (
    ...
    // Get base throttle time from optimal configuration
    const getBaseThrottleTime = (violationType: string): number => {
      switch (violationType) {
        case 'face_detection': return MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.faceDetection;
        case 'gaze_tracking': return MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.gazeTracking;
        case 'eye_movement': return MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.eyeMovement;
        case 'multiple_people': return MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.multiplePeople;
        case 'unauthorized_object': return MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.unauthorizedObject;
        default: return 3000; // Default 3 seconds
      }
    };

    // Progressive throttling - increase throttle time for repeat violations
    const lastViolation = lastViolationTime.current[violationKey];
    let throttleTime = getBaseThrottleTime(type);
    
    if (lastViolation) {
      const timeSinceLastViolation = now - lastViolation.time;
      
      // If this is a repeat violation within a reasonable timeframe, apply progressive multiplier
      if (timeSinceLastViolation < throttleTime * 3) { // Within 3x the throttle time
        const progressiveMultiplier = Math.min(
          MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.progressiveMultiplier,
          MEDIAPIPE_OPTIMAL_CONFIG.violationThrottling.maxProgressiveMultiplier
        );
        throttleTime *= progressiveMultiplier;
      }
      
      if (timeSinceLastViolation < throttleTime) {
        return; // Still in cooldown period
      }
    }

    // Check minimum confidence threshold
    if (confidence < MEDIAPIPE_OPTIMAL_CONFIG.confidence.violationMinimumConfidence) {
      return; // Confidence too low to report
    }
