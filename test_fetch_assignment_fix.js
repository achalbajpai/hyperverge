...
async function testFetchAssignmentFix() {
   ...
   try {
      ...
      if (courseTasksResponse.ok) {
         ...
         if (matchingTask) {
            console.log(
               `   Found matching task: ID ${matchingTask.id}, course_task_id ${matchingTask.course_task_id}`
            );
            ...
            if (actualTaskResponse.ok) {
               ...
               console.log(
                  "   ✅ Fix should work - assignment ID 10 can be resolved to task ID",
                  matchingTask.id
               );
            } else {
         ...
         console.log(
            `   ❌ Could not fetch course 3 tasks: ${courseTasksResponse.status}`
         );
      }
   } catch (error) {
      console.log(`   Error: ${error.message}`);
