...
warnings.filterwarnings(
    "ignore", category=UserWarning, message=".*torchaudio.*deprecated.*"
)
...
class SpeakerDetector:
    ...
    def _initialize_model(self):
        ...
        if not PYANNOTE_AVAILABLE or not TORCH_AVAILABLE:
            self.logger.error(
                "Required dependencies not available for speaker diarization"
            )
            return
        ...
        try:
            ...
            if not hf_token or hf_token == "hf_placeholder_token_to_suppress_warnings":
                # Use fallback capabilities silently
                return
    ...
    def _extract_speaker_features(
        self, audio_data: np.ndarray, segment: Tuple[float, float]
    ) -> Dict:
        """
        ...
        zero_crossing_rate = np.sum(np.diff(np.sign(segment_audio)) != 0) / len(
            segment_audio
        )
    ...
    def analyze_speaker_patterns(
        self, diarization: Annotation, audio_data: np.ndarray
    ) -> SpeakerAnalysis:
        """
        ...
        primary_speaker_ratio = (
            primary_duration / total_duration if total_duration > 0 else 0
        )
    ...
    def _identify_suspicious_patterns(
        ...
        if overlapping_duration > 5.0:  # 5 seconds of overlap
            patterns.append(
                f"Overlapping speech detected ({overlapping_duration:.1f}s)"
            )
    ...
    def _calculate_diarization_confidence(
        self, speakers: int, switches: int, primary_ratio: float
    ) -> float:
    ...
    async def analyze_audio_async(
        self, audio_data: np.ndarray, session_id: str, sample_rate: int = 16000
    ) -> Dict:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Speaker analysis completed for session {session_id}. "
    ...
    def get_fallback_analysis(self, audio_data: np.ndarray) -> Dict:
        ...
        return {
            ...
            "speaker_segments": [],
            "suspicious_patterns": (
                ["Fallback analysis - limited accuracy"]
    ...
    def get_session_summary(self) -> Dict:
        ...
        multi_speaker_sessions = sum(
            1 for h in self.speaker_history if h.get("total_speakers", 0) > 1
        )
        ...
        return {
            ...
            "multi_speaker_percentage": (
                (multi_speaker_sessions / total_sessions * 100)
                if total_sessions > 0
                else 0
            ),
