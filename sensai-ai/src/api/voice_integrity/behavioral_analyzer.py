...
class BehavioralAnalyzer:
    ...
    def _load_suspicious_phrases(self) -> List[str]:
        ...
        return [
            # English phrases
            "what is the answer", "help me with", "tell me", "can you help",
            "what should i write", "give me the answer", "how do i solve",
            "what's the solution", "i don't know this", "can you do this",
            "google it", "search for", "look it up", "check online",
            "ask someone", "call someone", "text someone",
            
            # Hindi phrases  
            "जवाब क्या है", "मदद करो", "बताओ", "हल क्या है",
            "उत्तर दो", "यह कैसे करते हैं", "गूगल करो",
            
            # Bengali phrases
            "উত্তর কি", "সাহায্য করো", "বলো", "সমাধান কি",
            
            # Telugu phrases
            "జవాబు ఏమిటి", "సహాయం చేయి", "చెప్పు", "పరిష్కారం ఏమిటి",
            
            # Tamil phrases
            "பதில் என்ன", "உதவி செய்", "சொல்லு", "தீர்வு என்ன",
        ]
    ...
    def analyze_speech_patterns(self, audio_data: np.ndarray, 
        ...
        speech_rate = (estimated_words / total_speech_time) * 60 if total_speech_time > 0 else 0
        ...
        pause_frequency = len(pause_durations) / total_speech_time if total_speech_time > 0 else 0
        ...
        speech_consistency = 1.0 - (np.std(segment_durations) / np.mean(segment_durations) 
    ...
    def analyze_content(self, transcription: str) -> Dict:
        ...
        for phrase in self.suspicious_phrases:
            if phrase.lower() in transcription_lower:
                ...
                suspicious_details.append({
                    'phrase': phrase,
                    'type': 'help_seeking',
                    'positions': [m.start() for m in re.finditer(re.escape(phrase.lower()), transcription_lower)]
                })
        ...
        question_reading = bool(re.search(r'\?.*\b(is|are|what|how|where|when|why)', transcription))
        
        # Detect external discussion (multiple speaker indicators)
        external_discussion = any(indicator in transcription_lower for indicator in [
            'he said', 'she said', 'they said', 'someone said',
            'person said', 'friend said', 'teacher said'
        ])
    ...
    def calculate_overall_risk_score(self, metrics: BehavioralMetrics) -> Tuple[float, List[str]]:
        """
        ...
        if metrics.help_seeking_phrases > 0:
            risk_score += 0.3
            risk_factors.append(f"Help-seeking phrases detected ({metrics.help_seeking_phrases})")
        
        if metrics.answer_receiving_phrases > 0:
            risk_score += 0.4
            risk_factors.append(f"Answer-receiving phrases detected ({metrics.answer_receiving_phrases})")
    ...
    def analyze_behavior(self, 
                        ...
                        voice_events: List[Dict]) -> BehavioralMetrics:
        """
        ...
        analysis_result['content_details'] = content_analysis.get('suspicious_phrase_details', [])
        
        self.analysis_history.append(analysis_result)
        
        self.logger.info(f"Behavioral analysis completed. Risk score: {risk_score:.3f}, "
    ...
    def get_analysis_summary(self) -> Dict:
        ...
        return {
            ...
            'average_risk_score': np.mean([a.get('risk_score', 0) for a in recent_analyses]),
            'total_risk_factors': sum(len(a.get('risk_factors', [])) for a in recent_analyses),
